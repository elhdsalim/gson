 ...
 * encoded value as a
 * stream of tokens. This stream includes both literal values (strings, numbers,
 * booleans, and
 * nulls) as well as the begin and end delimiters of objects and arrays. The
 * tokens are traversed in
 ...
 * #beginArray} to consume
 * the array's opening bracket. Then create a while loop that accumulates
 * values, terminating
 * when {@link #hasNext} is false. Finally, read the array's closing bracket by
 * calling {@link
 * #endArray}.
 * <li>Within <strong>object handling</strong> methods, first call {@link
 * #beginObject} to consume
 * the object's opening brace. Then create a while loop that assigns values to
 * local variables
 * based on their name. This loop should terminate when {@link #hasNext} is
 * false. Finally,
 ...
 * exception. Lenient
 * parsers should call {@link #skipValue()} to recursively skip the value's
 * nested tokens, which may
 * otherwise conflict.
 *
 * <p>If a value may be null, you should first check using {@link #peek()}. Null
 * literals can be
 ...
 *
 * <ul>
 * <li>{@link #setStrictness(Strictness)}, the default is {@link
 * Strictness#LEGACY_STRICT}
 * <li>{@link #setNestingLimit(int)}, the default is {@value
 * #DEFAULT_NESTING_LIMIT}
 * </ul>
 *
 * The default configuration of {@code JsonReader} instances used internally by
 * the {@link
 * com.google.gson.Gson} class differs, and can be adjusted with the various
 * {@link
 ...
 * be read as numbers.
 * For example, both elements of the JSON array {@code [1, "1"]} may be read
 * using either {@link
 * #nextInt} or {@link #nextString}. This behavior is intended to prevent lossy
 * numeric conversions:
 * double is JavaScript's only numeric type and very large values like {@code
 * 9007199254740993}
 * cannot be represented exactly on that platform. To minimize precision loss,
 * extremely large
 ...
 * request
 * forgery</a> attacks. In such an attack, a malicious site gains access to a
 * private JSON file by
 * executing it with an HTML {@code <script>} tag.
 *
 * <p>Prefixing JSON files with <code>")]}'\n"</code> makes them non-executable
 * by {@code <script>}
 * tags, disarming the attack. Since the prefix is malformed JSON, strict
 * parsing fails when it is
...
public class JsonReader implements Closeable {
   ...
   * so we can create
   * strings without an intermediate StringBuilder. We decode literals directly
   * out of this buffer,
   * so it must be at least as long as the longest token that can be reported as a
   * number.
   */
   ...
   *     JsonReader.setStrictness(Strictness.LENIENT)} and
   *             {@code JsonReader.setLenient(false)}
   *             should be replaced by
   *             {@code JsonReader.setStrictness(Strictness.LEGACY_STRICT)}.<br>
   *             However, if you used {@code setLenient(false)} before, you might
   *             prefer {@link
   *             Strictness#STRICT} now instead.
   * @param lenient whether this reader should be lenient. If true, the strictness
   *                is set to {@link
   *                Strictness#LENIENT}. If false, the strictness is set to
   *                {@link Strictness#LEGACY_STRICT}.
   * @see #setStrictness(Strictness)
   */
  @Deprecated
  // Don't specify @InlineMe, so caller with `setLenient(false)` becomes aware of
  // new
  // Strictness.STRICT
  ...
  /**
   * Returns true if the {@link Strictness} of this reader is equal to
   ...
   *
   * <p>
   * In {@linkplain Strictness#STRICT strict} mode, the parser only accepts JSON
   * in accordance
   * with <a href="https://www.ietf.org/rfc/rfc8259.txt">RFC 8259</a>. In
   * {@linkplain
   * Strictness#LEGACY_STRICT legacy strict} mode (the default), only JSON in
   * accordance with the
   * RFC 8259 is accepted, with a few exceptions denoted below for backwards
   * compatibility reasons.
   * In {@linkplain Strictness#LENIENT lenient} mode, all sort of non-spec
   * compliant JSON is
   * accepted (see below).
   *
   * <dl>
   ...
   * <dd>In legacy strict mode, the following departures from RFC 8259 are
   * accepted:
   * <ul>
   * <li>JsonReader allows the literals {@code true}, {@code false} and
   * {@code null} to have
   * any capitalization, for example {@code fAlSe} or {@code NULL}
   * <li>JsonReader supports the escape sequence {@code \'}, representing a
   * {@code '}
   * (single-quote)
   * <li>JsonReader supports the escape sequence <code>\<i>LF</i></code> (with
   * {@code LF}
   * being the Unicode character {@code U+000A}), resulting in a {@code LF} within
   * the
   * read JSON string
   * <li>JsonReader allows unescaped control characters ({@code U+0000} through
   * {@code
   *             U+001F})
   * </ul>
   * <dt>{@link Strictness#LENIENT}
   * <dd>In lenient mode, all input that is accepted in legacy strict mode is
   ...
   * prefix</a>,
   * {@code ")]}'\n"}
   * <li>Streams that include multiple top-level values. With legacy strict or
   * strict
   ...
   * </dl>
   *
   * @param strictness the new strictness value of this reader. May not be
   *                   {@code null}.
   * @see #getStrictness()
   * @since 2.11.0
   */
   ...
   *
   * <p>
   * The nesting limit defines how many JSON arrays or objects may be open at the
   * same time. For
   * example a nesting limit of 0 means no arrays or objects may be opened at all,
   * a nesting limit
   * of 1 means one array or object may be open at the same time, and so on. So a
   * nesting limit of 3
   * allows reading the JSON data <code>[{"a":[true]}]</code>, but for a nesting
   * limit of 2 it would
   * fail at the inner {@code [true]}.
   *
   * <p>
   * The nesting limit can help to protect against a {@link StackOverflowError}
   * when recursive
   * {@link com.google.gson.TypeAdapter} implementations process deeply nested
   * JSON data.
   *
   * <p>
   ...
   * array.
   *
   * @throws IllegalStateException if the next token is not the beginning of an
   *                               array.
   */
  ...
  private int peekLiteral() throws IOException {
  ...
  private void handleNextNonWhitespace() throws IOException {
    int c = nextNonWhitespace(true);
    switch (c) {
      case ':':
        break;
  ...
  private int returnPeekNumber(long value, boolean negative, boolean fitsInLong, int last, int i) {
    ...
    // Don't try to convert Long.MIN_VALUE to positive long; it would overflow
    // MAX_VALUE
  ...
  private int peekNumber() throws IOException {
    ...
    return returnPeekNumber(value, negative, fitsInLong, last, i);
  }
  ...
  /**
   * Returns the next token, a {@link JsonToken#NAME property name}, and consumes
   * it.
   *
   ...
   * consuming it. If the next
   * token is a string, this method will attempt to parse it as a double using
   * {@link
   * Double#parseDouble(String)}.
   *
   * @throws IllegalStateException  if the next token is neither a number nor a
   *                                string.
   * @throws NumberFormatException  if the next literal value cannot be parsed as
   *                                a double.
   * @throws MalformedJsonException if the next literal value is NaN or Infinity
   *                                and this reader is
   *                                not {@link #setStrictness(Strictness)
   *                                lenient}.
   */
  ...
  /**
   * Returns the {@link JsonToken#NUMBER long} value of the next token, consuming
   * it. If the next
   * token is a string, this method will attempt to parse it as a long. If the
   * next token's numeric
   * value cannot be exactly represented by a Java {@code long}, this method
   * throws.
   *
   * @throws IllegalStateException if the next token is neither a number nor a
   *                               string.
   ...
   * character escape
   * sequences encountered along the way. The opening quote should have already
   * been read. This
  ...
  private String nextQuotedValue(char quote) throws IOException {
    ...
    while (true) {
      ...
      while (p < l) {
        ...
        // In strict mode, throw an exception when meeting unescaped control characters
        // (U+0000
        // through U+001F)
  ...
  /**
   * Returns the {@link JsonToken#NUMBER int} value of the next token, consuming
   * it. If the next
   * token is a string, this method will attempt to parse it as an int. If the
   * next token's numeric
   * value cannot be exactly represented by a Java {@code int}, this method
   * throws.
   *
   * @throws IllegalStateException if the next token is neither a number nor a
   *                               string.
  ...
  /**
   * Skips the next value recursively. This method is intended for use when the
   * JSON token stream
   ...
   *
   * <ul>
   * <li>Start of a JSON array or object: It and all of its nested values are
   * skipped.
   * <li>Primitive value (for example a JSON number): The primitive value is
   * skipped.
   * <li>Property name: Only the name but not the value of the property is
   * skipped. {@code
   ...
   * <li>End of JSON document: Skipping has no effect, the next token continues to
   * be the end of
   * the document.
   * </ul>
   */
    ...
    do {
      ...
      switch (p) {
        ...
        case PEEKED_END_ARRAY:
          ...
          break;
        case PEEKED_END_OBJECT:
          // Only update when object end is explicitly skipped, otherwise stack is not
          // updated
          // anyways
          if (count == 0) {
        ...
        case PEEKED_UNQUOTED_NAME:
          skipUnquotedValue();
          // Only update when name is explicitly skipped, otherwise stack is not updated
          // anyways
          if (count == 0) {
        ...
        case PEEKED_EOF:
          ...
          return;
        default:
          // For all other tokens there is nothing to do; token has already been consumed
          // from
          // underlying reader
      }
      peeked = PEEKED_NONE;
  ...
  private void push(int newTop) throws MalformedJsonException {
    // - 1 because stack contains as first element either EMPTY_DOCUMENT or
    // NONEMPTY_DOCUMENT
   ...
   * part of a comment.
   * When this returns, the returned character is always at {@code buffer[pos-1]};
   * this means the
   * caller can always push back the returned character by decrementing
   * {@code pos}.
   */
  ...
  /**
   * Returns a <a href="https://goessner.net/articles/JsonPath/">JSONPath</a> in
   * <i>dot-notation</i>
   * to the next (or current) location in the JSON document. That means:
   *
   * <ul>
   * <li>For JSON arrays the path points to the index of the next element (even if
   * there are no
   * further elements).
   ...
   * <i>before</i> a
   * value is consumed, for example when the {@linkplain #peek() peeked} token is
   * unexpected.
   */
   ...
   * If no element has been consumed yet it uses the index 0 (even if there are no
   * elements).
   * <li>For JSON objects the path points to the last property, or to the current
   * property if its
   * name has already been consumed.
   ...
   * backslash. The backslash '\' should have already been read. This supports
   * both Unicode escapes
  ...
  /**
   * Throws a new {@link MalformedJsonException} with the given message and
